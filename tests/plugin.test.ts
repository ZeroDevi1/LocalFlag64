...
const originalRepositoryEnv = process.env.GITHUB_REPOSITORY;

afterEach(() => {
  if (originalRepositoryEnv === undefined) {
    delete process.env.GITHUB_REPOSITORY;
    return;
  }

  process.env.GITHUB_REPOSITORY = originalRepositoryEnv;
});

describe("plugin rendering", () => {
  it("生成包含 Script 和 MITM 的插件文件", () => {
    process.env.GITHUB_REPOSITORY = "ZeroDevi1/LocalFlag64";
    ...
    expect(plugin).toContain("#!homepage= https://github.com/example/localflag64");
  });
